...
static void	process_conv_g(t_ctx *ctx, t_conv *conv, t_pf_float *fp);
...
void	pf_conv_g_binary64(va_list *ap, t_ctx *ctx, t_conv *conv)
{
	...
	if (conv->has_prec == false)
		conv->precision = 6;
	if (conv->has_prec == true && conv->precision == 0)
		conv->precision = 1;
...
static void	process_conv_g(t_ctx *ctx, t_conv *conv, t_pf_float *fp)
{
	...
	if (decimal_exp < -4 || conv->precision <= decimal_exp)
		pf_print_conv_g_exp_notation(ctx, conv, fp, decimal_exp);
	else
		pf_print_conv_g_fixed_point(ctx, conv, fp, decimal_exp);
}
