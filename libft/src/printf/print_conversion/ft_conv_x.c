...
static void		ft_print_x(t_printf_ctx *ctx, uintmax_t num);
static size_t	ft_get_digits_uintmax_hexa(uintmax_t num);
static void		ft_print_hexa(t_printf_ctx *ctx, uintmax_t num);
...
static void	ft_print_hexa(t_printf_ctx *ctx, uintmax_t num)
{
	...
	if (ctx->conv == 'x')
		ft_xprint_char(ctx, "0123456789abcdef"[num % 16]);
	else
		ft_xprint_char(ctx, "0123456789ABCDEF"[num % 16]);
}
