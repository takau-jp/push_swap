...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/21 03:25:56 by stanaka2          #+#    #+#             */
...
static void		ft_print_a(t_printf_ctx *ctx, t_my_double_hex *my_double_hex);
...
void	ft_conv_a_binary64(va_list *ap, t_printf_ctx *ctx)
{
	...
	if (my_double_hex.exponent == 0)
		ft_normalize_hex_binary64(&my_double_hex);
				...
				t_printf_ctx *ctx, t_my_double_hex *my_double_hex)
{
	...
	ft_xprint_char(ctx, my_double_hex->hex_int + '0');
	if (!(ctx->precision == 0 && !ctx->hash_flag))
		ft_xprint_char(ctx, '.');
	...
	{
		if (ctx->conv == 'a')
			ft_xprint_char(ctx, "0123456789abcdef"[my_double_hex->hex_frac[i]]);
		else
			ft_xprint_char(ctx, "0123456789ABCDEF"[my_double_hex->hex_frac[i]]);
		i++;
		precision--;
	...
	ft_print_exponent_hex(ctx, my_double_hex->exponent - DBL_EMAX);
}
