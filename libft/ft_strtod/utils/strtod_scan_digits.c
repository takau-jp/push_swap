	...
	while (ft_isdigit(**nptr) || (**nptr == '.' && (radix_point == *nptr)))
	{
		if (**nptr != '0' && **nptr != '.')
		{
			if (0 < shift && LONG_MIN + shift > i)
				...
				add_digit(to_double, i - shift, (uint8_t)(**nptr - '0'));
		}
		if (**nptr != '.')
			++i;
		++(*nptr);
...
void	strtod_scan_hex_digits(const char **nptr, t_to_double *to_double)
{
	...
	const int8_t	mod = (int8_t)(exponent % 4);
	const char		*radix_point;
	ptrdiff_t		i;
	...
	t_to_double *to_double, ptrdiff_t i, char c, int8_t mod)
{
	...
	if (mod > 0)
	{
		...
		if (LONG_MIN < i)
			add_digit(to_double, i - 1, (uint8_t)((n * power) / 16));
	}
	else if (mod < 0)
	{
		add_digit(to_double, i, n / power);
		if (i < LONG_MAX)
			add_digit(to_double, i + 1, (uint8_t)(((n % power) * 16) / power));
	}
	else
