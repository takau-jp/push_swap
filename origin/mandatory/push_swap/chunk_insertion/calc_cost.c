...
/*   Updated: 2025/09/28 16:29:06 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_ctx *ctx, t_element *element, t_rotate_cost *rotate_cost, t_rotate_func rotate)
{
	...
	if (rotate_cost->b < rotate_cost->a_same)
	{
		if (rotate_cost->a_same < rotate_cost->a_reverse + rotate_cost->b)
		{
			...
			if (rotate != up_rotate)
				cost += biased_rotate_cost(ctx, 0, rotate_cost->a_same);
			else
				cost += biased_rotate_cost(ctx, rotate_cost->b, 0);
		}
		else
		{
			...
			if (rotate == up_rotate)
				cost += biased_rotate_cost(ctx, rotate_cost->b, rotate_cost->a_reverse);
		}
	}
	else
	{
...
ssize_t	biased_rotate_cost(t_ctx *ctx, ssize_t rb_cost, ssize_t rra_cost)
{
	...
	biased_cost = rb_cost * ctx->rotate_b_bias_factor;
