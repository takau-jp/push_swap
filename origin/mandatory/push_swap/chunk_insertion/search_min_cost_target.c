...
/*   Updated: 2025/09/28 15:00:30 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_target_element *target, ssize_t *min_cost, t_rotate_func rotate)
{
	...
	{
		// printf("next index: %zu, b_up: %d\n", element->index, rotate == up_rotate);
		if (ctx->is_last_chunk == false)
			cost = calc_cost(ctx, element, &rotate_cost, rotate);
		else
			cost = calc_cost_last_chunk(ctx, element, &rotate_cost, rotate);
		if (cost < *min_cost \
